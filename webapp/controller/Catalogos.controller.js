  ...
  function (
    ...
    return Controller.extend("com.cat.sapfioricatalogs.controller.Catalogos", {
      ...
      onSociedadChange: function (oEvent) {
        ...
        const sSelectedSociedadKey = oSource.getSelectedKey();

        console.log("onSociedadChange - Selected Key:", sSelectedSociedadKey);
        ...
        if (sSourceId.includes("updateInputIdSociedad")) {
          ...
          this.byId("updateInputIdCedi").setEnabled(!!sSelectedSociedadKey);
        } else {
          this.byId("inputIdCedi").setSelectedKey(null);
          this.byId("inputIdCedi").setEnabled(!!sSelectedSociedadKey);
        }
      },
      ...
      _filterCedis: function (sParentKey) {
        ...
        console.log("Filtering CEDIs for parent key:", sParentKey);
        console.log("All CEDIs:", aAllCedis);
        ...
        console.log("Filtered CEDIs:", aFilteredCedis);
      ...
      onUpdate: function () {
        ...
        oUpdateData.originalIdEtiqueta = oUpdateData.idetiqueta;
        oUpdateData.originalIdValor = oUpdateData.idvalor;
      ...
      onSaveUpdate: function (oEvent) {
        ...
        if (updatedData.parent === true) {
          if (!updatedData.idetiqueta || updatedData.idetiqueta.trim() === "") {
            MessageBox.error("El campo 'ID Etiqueta' no puede estar vacío.");
            return;
          }
          ...
          if (!updatedData.idvalor || updatedData.idvalor.trim() === "") {
            MessageBox.error("El campo 'ID Valor' no puede estar vacío.");
            return;
          }
        ...
        if (updatedData.parent) {
          const updates = {
            ...
            IDETIQUETA: updatedData.idetiqueta,
          ...
          operation = {
            ...
            action: "UPDATE",
            payload: {
              id: updatedData.originalIdEtiqueta,
              updates: updates,
            },
          };
        } else {
          ...
          operation = {
            ...
            action: "UPDATE",
            payload: {
              id: updatedData.originalIdValor,
              updates: updates,
            },
          };
        }
        ...
        if (updatedData.parent) {
          const updatedLabels = labels.map((label) =>
            label.idetiqueta === updatedData.originalIdEtiqueta
              ? updatedData
              : label
          );
          dataModel.setProperty("/labels", updatedLabels);
          ...
          const updatedLabels = labels.map((label) => {
            if (
              ...
              return {
                ...label,
                children: (label.children || []).map((child) =>
                  child.idvalor === updatedData.originalIdValor
                    ? { ...updatedData }
                    : child
                ),
              };
      ...
      onSearch: function (oEvent) {
        ...
        aMasterData.forEach(parent => {
          ...
          if (parentMatches) {
            ...
            const parentClone = JSON.parse(JSON.stringify(parent));
            parentClone.children = childrenMatches;
            aFilteredData.push(parentClone);
          }
        });
